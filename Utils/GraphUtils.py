...
def markUsingPartitionList(partitionList):
    marked_nodes = []
    pCount = 1
    for p in partitionList:
        nodes = p.nodes(data=True)
        for n in nodes:
            nList = getContractedNodes(n[0])
            for n in nList:
                # marked_nodes.append({'nodeID':n, 'partition':pCount})
                marked_nodes.append((n, {'partition':pCount}))
        pCount+=1
        # break
    ...
    for m in marked_nodes:
        # print(m)
        if(m[1].get('partition') ==1):
            c1+=1
        elif(m[1].get('partition') ==2):
            c2+=1
    # print(c1,c2)
    return marked_nodes
