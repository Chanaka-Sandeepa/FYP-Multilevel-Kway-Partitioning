...
ox.config(log_file=True, log_console=True, use_cache=False)
...
def get_colombo_graph_from_file():
    G = ox.graph_from_address('Colombo, Colombo District, Western Province, Sri Lanka', distance=35000, distance_type='bbox', network_type='drive', simplify=True, retain_all=False,
                              truncate_by_edge=False, infrastructure='way["highway"]')
    # G_projected = ox.project_graph(G)

    ox.save_graphml(G, filename='colombo_large.graphml')
    r_graph = nx.read_graphml("data/colombo_large.graphml")
    r_graph = (convert_to_simpleG(r_graph))

    return r_graph,G


def get_manhatten_graph_from_file():
    # G = ox.graph_from_address('Manhattan Island, New York City, New York, USA', network_type='drive', simplify=True, retain_all=False,truncate_by_edge=False, infrastructure='way["highway"]')
    G = ox.graph_from_place('Manhattan Island, New York City, New York, USA', network_type='drive', infrastructure='way["highway"]')
    # G_projected = ox.project_graph(G)

    ox.save_graphml(G, filename='manhatten1.graphml')
    r_graph = nx.read_graphml("data/manhatten1.graphml")
    r_graph = (convert_to_simpleG(r_graph))
